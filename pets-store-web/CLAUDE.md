...
src/
  ...
  store/index.js       # configureStore: { animal, auth }
  features/
    header/            # шапка
    animal/model/      # redux-логика:
      animal/          #   animal.slice.js, animal.thunks.js (fetchAnimals, fetchCategories)
      auth/            #   auth.slice.js,  auth.thunk.js  (loginAuth, registerAuth, fetchMe)
  pages/
    home/              # home.page.jsx + components/ (animal-card, filter, photo-gallery)
    login/             # login.page.jsx
    register/          # register.page.jsx (регистрация покупателя/продавца)
```

### Состояние (Redux Toolkit)
- Два слайса: `animal` и `auth`. Асинхронные операции — `createAsyncThunk` + `axios`.
- `animal`: список животных, категории, фильтры (`categoryId`, `search`), сортировка, `currentAnimal` (для модалки), корзина.
- `auth`: `accessToken`, данные пользователя.
